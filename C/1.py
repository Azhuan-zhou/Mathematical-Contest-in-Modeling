...
def draw_win_prob(data,points,point_1):
    ...
    for i in range(0,point_1):
        point = points[i]
        plt.scatter(point[0]-1,point[1],color='b', label="Player 1's swing")
    for i in range(point_1,len(points)):
        point = points[i]
        plt.scatter(point[0]-1,point[1],color='b', label="Player 2's swing")
    # 绘制垂直虚线
...
data_raw = pd.read_csv('Wimbledon_featured_matches.csv')
...
p2_p = 0.6
n = 200
performance =  pd.read_csv('probability1.csv')
